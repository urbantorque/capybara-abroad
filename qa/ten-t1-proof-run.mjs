// ROADMAP-TEN T1 proof slot: a T1 builder's playwright-cli instrument, run
// once more on the merged tree, headful Edge on the reference GPU, rung 0.
//   CAPY_QA_URL=http://localhost:5199/ node qa/ten-t1-proof-run.mjs qa/ten-t1c-board.js
...
// back, and each picture and sink name gains a `ten-t1-proof-` prefix so the
// builders' own results are not overwritten.
import { readFileSync } from 'node:fs';
import { openHarness } from './reimagine-harness.mjs';

const file = process.argv[2];
if (!file) throw new Error('usage: node qa/ten-t1-proof-run.mjs <qa/script.js>');
...
const src = readFileSync(file, 'utf8')
  ...
  .replace(/const PORT = 51\d\d/g, 'const PORT = ' + port)
  .replace(/perfRung = 3/g, 'perfRung = 0')
  .replace(/localStorage\.clear\(\)/g, `(localStorage.clear(), ${PIN})`)
  .replace(/'qa\/'/g, `'qa/ten-t1-proof-'`)
  .replace(/path: 'qa\/(?!ten-t1-proof-)/g, `path: 'qa/ten-t1-proof-`)
  .replace(/\/shot\?name='/g, `/shot?name=ten-t1-proof-'`)
  .replace(/\/shot\?name=(?!ten-t1-proof-)(?=[\w-])/g, '/shot?name=ten-t1-proof-');
