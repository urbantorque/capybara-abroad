async page => {
  ...
  const chapters = ['sydney', 'kyoto', 'rio', 'palawan', 'iceland']
  for (const ch of chapters) {
    ...
    row.still = await meter(12, 'still')
  ...
  await page.evaluate(async (o) => {
    await fetch('/shot?name=l7r-audio-states.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) })
  }, out)
}
