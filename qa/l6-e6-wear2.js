async page => {
  ...
  const CH = {
    ...
    palawan: { pts: [0, 44.5, 2.7, 36.0, 6, 29.2], api: 'palawan' },
  }
  const out = { rows: [] }
  for (const c of Object.keys(CH)) {
    ...
    await page.waitForTimeout(6000)
    await page.screenshot({ path: 'qa/l6-e6-wear2-' + c + '.png' })
  ...
  await page.evaluate((o) => fetch('/shot?name=l6-e6-wear2.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) }), out)
}
