async page => {
  ...
  for (let ci = 0; ci < NAMES.length; ci++) {
    ...
    const r = await page.evaluate(async () => {
      ...
      // the pass's own INTENDED motion — mist drift, grass sway, the cloud band,
      // the rays' breathing, the mirror's wobble, the petal skitter — is cut for
      // both frames so what is left is the floor: thin things, specks, texels.
      const FL = ['noMist', 'noGrass', 'noSky2', 'noRays', 'noReflect', 'noSkitter', 'noFleetGlow', 'noCaustic']
      const was = {}
      for (const f of FL) { was[f] = g.state[f]; g.state[f] = true }
      ...
      for (const f of FL) g.state[f] = was[f]
      return Object.assign({ biome: g.biome && g.biome.current, hidden: hidden.length, W, H }, best)
    })
    out.rows[name] = r
  }
  await page.evaluate(async (o) => { await fetch('/shot?name=wow-still-after.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o, null, 1)))) }) }, out)
}
