async page => {
  ...
  // THE A/B (L7, E4): the same settled frame with each term cut in turn, so the
  // before is read off the same pixels under whatever lens is live.
  const info = async (tag) => page.evaluate((tag) => {
    ...
    return { tag, biome: g.biome.current, camPitch: +(pitch * 180 / Math.PI).toFixed(1), horizonFromTop: +hz.toFixed(2),
      dist: +(ci.dist || 0).toFixed(1), rest: +(ci.rest || 0).toFixed(2), capyPx, cam: g.camera.position.toArray().map(v => +v.toFixed(2)),
  ...
  const audit = async () => page.evaluate(() => {
    ...
    return { live, localsN: L.length, spread, names, err: g.state.lastError || null }
  })
  const sw = async (k, v) => page.evaluate(([k, v]) => { window.__capy.state[k] = v }, [k, v])
  const shot = async (c, tag) => { await page.waitForTimeout(350); await page.screenshot({ path: 'qa/l7-e4-ab-' + c + '-' + tag + '.png' }); out.rows.push(await info(c + '-' + tag)) }
  const CH = ['cave', 'venice', 'quay', 'sahara', 'kowloon']
  for (const c of CH) {
    await page.evaluate((c) => window.__capy.hud.cross(c), c)
    await page.waitForTimeout(9000)
    if (c === 'venice') {
      // the reviewer's walk-stop-turn loop, so the frame is a calle
    ...
    await shot(c, 'on')
    await sw('noTri', true); await shot(c, 'notri'); await sw('noTri', false)
    await sw('noKey', true); await shot(c, 'nokey'); await sw('noKey', false)
    if (c === 'sahara') { await sw('noExposure', true); await shot(c, 'noexp'); await sw('noExposure', false) }
    if (c === 'quay' || c === 'kowloon') { await sw('noMirror', true); await shot(c, 'nomirror'); await sw('noMirror', false) }
    out.scene[c] = await audit()
  }
  await page.evaluate((o) => fetch('/shot?name=l7-e4-ab.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o, null, 1)))) }), out)
}
