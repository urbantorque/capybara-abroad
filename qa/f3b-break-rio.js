async page => {
  ...
  await wait(3500);
  await page.evaluate(() => { try { window.__capy.hud.cross('rio'); } catch (e) { window.__capy.biome.switchTo('rio'); } });
  await wait(9000);
  ...
  await page.evaluate(s => fetch('/shot?name=f3b-break-rio.json', { method: 'POST', body: s }), bl);
}
