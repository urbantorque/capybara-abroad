async page => {
  ...
  const CH = ['sydney', 'quay', 'venice', 'sahara', 'kowloon', 'cave']
  for (const c of CH) {
    if (c !== 'sydney') {
  ...
  await page.evaluate((o) => fetch('/shot?name=l7r-art-scene.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) }), out)
}
