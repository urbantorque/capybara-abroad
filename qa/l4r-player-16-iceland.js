async page => {
  ...
  await follow('glacier-run', 100, 'l4r-player-iceland-gl', 2.5, false)
  await snap('l4r-player-iceland-gl-top')
  // take the ice: run and hold G
  await page.keyboard.down('ShiftLeft'); await page.keyboard.down('KeyW'); await page.waitForTimeout(700); await page.keyboard.down('KeyG')
  for (let i = 0; i < 5; i++) { await page.waitForTimeout(2500); await snap('l4r-player-iceland-slide-' + i) }
  await page.keyboard.up('KeyG'); await page.keyboard.up('KeyW'); await page.keyboard.up('ShiftLeft')
  await page.waitForTimeout(2000); await snap('l4r-player-iceland-slide-end')
  out.lines = await drain()
  await post('l4r-player-16.json', out)
}
