async page => {
  ...
  out.leave = await page.evaluate(async () => {
    ...
    for (let i = 0; i < 1200; i++) {
      ...
      if (i % 60 === 0) {
        ...
        steps.push({ i, capyD: +sd.toFixed(2), gSt: gr ? gr.gSt : null, visible: gr && gr.group ? gr.group.visible : null })
      }
    }
    let gr = null
    for (const l of g.locals) if (l.trav && l.gateChap && l.biome === 'kyoto') gr = l
  ...
  await page.evaluate((o) => {
    ...
    fetch('/shot?name=wow2-glimpse-leave2', { method: 'POST', headers: { 'Content-Type': 'text/plain' }, body: enc })
  }, out)
}
