async page => {
  ...
  await follow('glacier-run', 75, 'l4r-player-iceland-g2', 2.5, false)
  await snap('l4r-player-iceland-g2-top')
  ...
  for (let i = 0; i < 6; i++) { await page.waitForTimeout(2500); await snap('l4r-player-iceland-s2-' + i) }
  await page.keyboard.up('KeyG'); await page.keyboard.up('KeyW'); await page.keyboard.up('ShiftLeft')
  await page.waitForTimeout(2000); await snap('l4r-player-iceland-s2-end')
  out.lines = await drain()
  await post('l4r-player-17.json', out)
}
