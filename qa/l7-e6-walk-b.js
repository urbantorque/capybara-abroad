async page => {
  ...
  const CH = ['sahara', 'drift', 'venice', 'kowloon', 'palawan', 'goreme']
  for (const b of CH) {
    if (b !== 'sydney') {
  ...
  await page.evaluate((o) => fetch('/shot?name=l7-e6-walk-b.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) }), log)
}
