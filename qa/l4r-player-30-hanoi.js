async page => {
  ...
  // one more go at the spritz: hop onto the table and press E
  for (let i = 0; i < 3; i++) { await page.keyboard.down('KeyW'); await page.waitForTimeout(150); await page.keyboard.press('Space'); await page.waitForTimeout(500); await page.keyboard.up('KeyW'); await page.keyboard.press('KeyE'); await page.waitForTimeout(700) }
  await snap('l4r-player-venice-sp2')
  ...
  await install()
  await snap('l4r-player-hanoi-01')
  await page.waitForTimeout(4000)
  await snap('l4r-player-hanoi-02')
  // a new player walks straight into the road
  await page.keyboard.down('KeyW')
  for (let i = 3; i < 7; i++) { await page.waitForTimeout(2500); await snap('l4r-player-hanoi-0' + i) }
  await page.keyboard.up('KeyW')
  await page.keyboard.press('KeyQ'); await page.waitForTimeout(2000)
  await snap('l4r-player-hanoi-07')
  out.lines = await drain()
  await post('l4r-player-30.json', out)
}
