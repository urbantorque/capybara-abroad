async page => {
  ...
  for (const b of ['sydney', 'quay', 'pasto']) {
    await page.evaluate((name) => {
      ...
      // The same detector with the place pinned HOT, to show heat adds no
      // state and no stall. See qa/BATCH7.md job 2.
      if (typeof g.forceHeat === 'function') g.forceHeat(1);
    }, b);
  ...
  await page.evaluate(async (o) => {
    await fetch('/shot?name=B7-nph-hot.json', { method: 'POST',
