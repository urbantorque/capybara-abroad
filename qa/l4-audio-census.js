async page => {
  // l4-audio-census (L4, E2: qa/l4r-audio-census.js re-run for the loudness order; see the tally at the end): every chapter, 30 s standing still at the spawn after the
  ...
  // THE LOUDNESS ORDER (the audio review's note to the room): the four band
  // chapters against the three hush chapters, and who the loudest two are.
  const bands = ['cali', 'rio', 'venice', 'monaco'], hush = ['cave', 'pasto', 'manly']
  const rms = n => (out[n] && out[n].rms) || -99
  const order = names.slice().sort((a, b) => rms(b) - rms(a))
  out.tally = {
    bandMin: Math.min(...bands.map(rms)), hushMax: Math.max(...hush.map(rms)),
    bandsOverHushBy: +(Math.min(...bands.map(rms)) - Math.max(...hush.map(rms))).toFixed(1),
    loudestTwo: order.slice(0, 2), order: order.map(n => n + ':' + rms(n)),
  }
  await page.evaluate(async (o) => {
    await fetch('/shot?name=l4-audio-census.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) })
  }, out)
}
