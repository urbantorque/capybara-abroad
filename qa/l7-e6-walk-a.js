async page => {
  ...
  const CH = ['sydney', 'pasto', 'quay', 'kyoto', 'cali', 'rio', 'iceland']
  for (const b of CH) {
    if (b !== 'sydney') {
  ...
  await page.evaluate((o) => fetch('/shot?name=l7-e6-walk-a.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) }), log)
}
