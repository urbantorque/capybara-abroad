async page => {
  ...
  await snap('l4r-player-iceland-12')
  await follow('geysir', 70, 'l4r-player-iceland-gey', 1.2, false)
  for (let i = 0; i < 5; i++) { await page.waitForTimeout(4000); await snap('l4r-player-iceland-geywait-' + i) }
  out.lines = await drain()
  await post('l4r-player-14.json', out)
}
