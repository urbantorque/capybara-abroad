async page => {
  ...
  const out = await page.evaluate(async () => {
    ...
    await fetch('/shot?name=D7-lit.png', { method:'POST', body: g.renderer.domElement.toDataURL('image/png').split(',')[1] })
    g.camera.position.set(4, 128, -146)
    g.camera.lookAt(new THREE.Vector3(36, 110, -190))
    g.camera.updateMatrixWorld(true)
    g.renderer.render(g.scene, g.camera)
    await fetch('/shot?name=D7-lit-wide.png', { method:'POST', body: g.renderer.domElement.toDataURL('image/png').split(',')[1] })
  ...
  await page.evaluate(async (o) => { await fetch('/shot?name=dri7.json', { method:'POST', body: btoa(JSON.stringify(o)) }) }, out)
}
