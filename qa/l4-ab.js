async page => {
  ...
  for (const n of ['hanoi', 'goreme', 'monaco', 'kyoto']) {
    ...
    // THE PAIR. The sweep (sysShadowCull) marks every mesh it turned off, so
    // the "before" is the same frame with those handed back — taken first,
    // then the sweep is allowed to run again, and the "after" is taken.
    if (n === 'hanoi' || n === 'kyoto') {
      await page.evaluate(() => { const g = window.__capy; let k = 0; g.scene.traverse(o => { if (o.userData && o.userData.sysSmallCast) { o.castShadow = true; k++; } }); g.__l4pairN = k; });
      await page.waitForTimeout(400);
      await page.screenshot({ path: 'qa/l4-' + n + '-before.png' });
      await page.evaluate(() => { const g = window.__capy; let k = 0; g.scene.traverse(o => { if (o.userData && o.userData.sysSmallCast) { o.castShadow = false; k++; } }); });
      await page.waitForTimeout(400);
      await page.screenshot({ path: 'qa/l4-' + n + '-after.png' });
    }
  ...
  await page.evaluate(async (o) => {
    await fetch('/shot?name=l4-ab.json', { method: 'POST',
