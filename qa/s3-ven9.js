async page => {
  ...
  const dry = await cam({ biome:'venice', warm:120, px:-4, py:1.6, pz:-30, cx:-4, cy:4.0, cz:-18, tx:-4, ty:1.6, tz:-50, name:'V9-dry.png' })
  const wet = await cam({ biome:'venice', warm:60*118, px:-4, py:1.6, pz:-30, cx:-4, cy:4.0, cz:-18, tx:-4, ty:1.6, tz:-50, name:'V9-wet.png' })
  await page.evaluate(async (o) => { await fetch('/shot?name=v9info.json', { method:'POST', body: btoa(JSON.stringify(o)) }) }, {dry, wet})
}
