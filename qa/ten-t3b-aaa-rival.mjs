...
//   node qa/ten-t3b-aaa-rival.mjs [chapter]
// T3b: A4's own seven checks, unchanged, with the T2a grace cut after arrival (a fresh
// Free Roam file is in its grace until its first repertoire name, so rivalOK is false
// and the bird never comes; measured: 'A states off'). Prints graceOn and the act first.
...
try {
  ...
  console.log('grace', await h.page.evaluate(() => window.__capy.graceOn()), 'act', await h.page.evaluate(() => window.__capy.journeyAct()), 'arc', await h.page.evaluate(() => window.__capy.rivalAudit().arcMode))
