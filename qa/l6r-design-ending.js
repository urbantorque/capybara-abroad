async page => {
  ...
  for (let i = 0; i < 70; i++) {
    const s = await page.evaluate(() => { const g = window.__capy; const p = g.capy.position; const led = document.querySelector('.capyui-led'); return { t: +g.state.time.toFixed(1), x: +p.x.toFixed(1), z: +p.z.toFixed(1), loaf: +(g.capy.loaf || 0).toFixed(2), dist: +g.camInfo.dist.toFixed(1), ended: !!g.state.ended, coda: g.hud.codaAudit ? g.hud.codaAudit() : null, led: led ? led.className : null, err: g.state.lastError || null } })
    out.samples.push(s)
    if (i === 6) await page.screenshot({ path: 'qa/' + TAG + '-03-sit.png' })
    if (i === 16) await page.screenshot({ path: 'qa/' + TAG + '-04-coda.png' })
