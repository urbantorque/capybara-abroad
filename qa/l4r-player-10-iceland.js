async page => {
  ...
  await snap('l4r-player-iceland-00-pause')
  const quit = await page.evaluate(() => { const b = [...document.querySelectorAll('button, div, span')].find(e => /QUIT TO THE TITLE/i.test(e.textContent) && e.children.length === 0); if (b) { b.click(); return true } return false })
  out.quit = quit
  ...
  await post('l4r-player-10.json', out)
}
