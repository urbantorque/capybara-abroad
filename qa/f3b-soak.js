async page => {
  ...
  await page.evaluate(() => {
    const KEYS = ['KeyW', 'KeyA', 'KeyS', 'KeyD', 'Space', 'KeyE', 'KeyQ', 'ShiftLeft', 'KeyG'];
    let s = 20260905 >>> 0;
  ...
  const snap = (name, lap) => page.evaluate(function (o) {
    ...
    return {
      ...
      scrapeG: (function(){try{return +g.hud.mixAudit().scrapeG.toFixed(4);}catch(e){return 'THREW';}})(),
      beatLen: (function(){try{return g.musAudit().beatLen;}catch(e){return 'THREW';}})(),
  ...
  await page.evaluate(s => fetch('/shot?name=f3b-soak.json', { method: 'POST', body: s }), bl);
}
