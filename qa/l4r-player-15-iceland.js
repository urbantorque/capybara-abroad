async page => {
  ...
  await snap('l4r-player-iceland-13')
  await follow([6, 40], 30, 'l4r-player-iceland-st', 3, false)
  await follow([8, 10], 25, 'l4r-player-iceland-st2', 3, false)
  await follow('geysir', 40, 'l4r-player-iceland-gey', 1.2, false)
  for (let i = 0; i < 6; i++) { await page.waitForTimeout(4000); await snap('l4r-player-iceland-geywait-' + i) }
  out.lines = await drain()
  await post('l4r-player-15.json', out)
}
