async page => {
  // L4: the settled frame of every chapter plus the title, for the fourth lift
  ...
  await page.screenshot({ path: 'qa/l4-title.png' })
  ...
  for (const key of KEYS) {
    ...
    await page.screenshot({ path: 'qa/l4-' + info.biome + '.png' })
    // a second frame after a short walk, so the animal is seen moving
    await page.keyboard.down('KeyW')
    await page.waitForTimeout(1500)
    await page.keyboard.up('KeyW')
    await page.waitForTimeout(400)
    await page.screenshot({ path: 'qa/l4-' + info.biome + '-walk.png' })
    out.rows.push(Object.assign({ key: key }, info))
  }
  out.errs = errs.slice(0, 40)
  await page.evaluate((o) => fetch('/shot?name=l4-shots.json', { method: 'POST',
