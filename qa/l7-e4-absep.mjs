// qa/l7-e4-absep.mjs — the animal's value separation from its surround, per
// settled frame. Reads the animal's projected box from qa/l7-e4-ab.json.png
...
//   node qa/l7-e4-absep.mjs
import { readFileSync } from 'node:fs';
import { pngRead } from './pnghist.mjs';
const rows = JSON.parse(readFileSync('qa/l7-e4-ab.json.png', 'utf8')).rows;
...
for (const r of rows) {
  const path = 'qa/l7-e4-ab-' + r.tag + '.png';
  ...
  console.log(r.tag.padEnd(18) + pad(am.toFixed(0), 5) + pad(rm.toFixed(0), 7) + pad((am - rm).toFixed(0), 5) + pad(asd.toFixed(0), 7) + pad(rsd.toFixed(0), 7) +
