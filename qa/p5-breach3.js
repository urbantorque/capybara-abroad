async page => {
  ...
  for (let k = 0; k < 5; k++) {
    const r = await page.evaluate(() => new Promise(res => {
      ...
      const t = setInterval(() => {
        ...
        const pod = g.antarctic.pod()
        g.scene.traverse(o => {
          if (!o.isMesh || !o.parent || o.parent.type !== 'Group') return
          if (Math.abs(o.position.x - pod.x) > 26 || Math.abs(o.position.z - pod.z) > 26) return
          if (Math.abs(o.position.y) > 12) return
  ...
  const done = await page.evaluate(() => [...document.querySelectorAll('li.done')].map(e=>e.textContent.trim()).filter(t=>/orca|spy|lead|arch/i.test(t)))
