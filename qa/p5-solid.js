async page => {
  ...
  for (const n of names) {
    ...
    out[n] = await page.evaluate((name) => {
      ...
      const B = { pantanal:[-120,120,-115,100], cave:[-58,58,-176,88], antarctic:[-200,180,-160,120] }[name]
      const api = g[name]
  ...
  await page.evaluate(async (o) => {
    await fetch('/shot?name=p5solid.json', { method:'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) })
  }, out)
}
