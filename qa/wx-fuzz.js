async page => {
  ...
  for (const n of names) {
    res[n] = await page.evaluate(async (name) => {
      ...
      // FORCE THE WEATHER. The stock fuzz never sees rain — a shower is a dice
      // roll on a 56-to-130-second timer and this soaks for eight — so the whole
      // wet path (the field, the slip, the coat, the splash, the umbrellas, the
      // bed) was invisible to it. Odds 1 and a 4 s gap means every chapter that
      // can rain is raining within a few seconds of arrival.
      if (g.weather) {
        const b = g.weather.rowOf(name);
        g.weather.set(name, { rain: { odds: 1, peak: Math.max(b.rain.peak, 0.55),
                                      hold: 90, gap: 4 } });
      }
  ...
  await page.evaluate(async (o) => {
    await fetch('/shot?name=wx-fuzz.json', { method: 'POST',
