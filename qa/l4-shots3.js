async page => {
  // L4 E1 (a1): the E1 set (L4-2b): the camera, the pools, the quay — qa/l4-shots.js verbatim, writing qa/l4c-*.png so
  ...
  if (!(await have('qa/l4c-title.png'))) await page.screenshot({ path: 'qa/l4c-title.png', timeout: 90000 })
  ...
  for (const key of KEYS) {
    await page.goto('http://localhost:5188/')
    if (await have('qa/l4c-' + NAMES[KEYS.indexOf(key)] + '-walk.png')) continue
    await page.waitForTimeout(5200)
    ...
    await page.screenshot({ path: 'qa/l4c-' + info.biome + '.png', timeout: 90000 })
    // a second frame after a short walk, so the animal is seen moving
    await page.keyboard.down('KeyW')
    await page.waitForTimeout(1500)
    await page.keyboard.up('KeyW')
    await page.waitForTimeout(400)
    await page.screenshot({ path: 'qa/l4c-' + info.biome + '-walk.png', timeout: 90000 })
