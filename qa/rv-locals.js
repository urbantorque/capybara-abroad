async page => {
  ...
  const names = ['sydney','quay','pasto','kyoto']
  const out = {}
  ...
  await page.evaluate(async (o) => { await fetch('/shot?name=rv-locals.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) }) }, out)
}
