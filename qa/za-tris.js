  ...
  for (const n of ['palawan','goreme']) {
    out[n] = await page.evaluate((name) => {
      ...
      return { tris: Math.round(tris), shadowTris: Math.round(shadowTris), meshes,
               ...
               top: per.slice(0,26), ghosts: ghosts.slice(0,20), err: g.state.lastError || null }
    }, n)
  }
  await page.evaluate(async (o) => { await fetch('/shot?name=zatris.json', { method:'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o, null, 1)))) }) }, out)
}
