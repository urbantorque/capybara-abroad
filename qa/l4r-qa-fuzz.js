async page => {
  ...
  for (const n of names) {
    res[n] = await page.evaluate(async (name) => {
      ...
      const held = new Set();
      // frame-time sampler on the real rAF loop, and the tick's own CPU cost
      const ft = [], tk = [];
      const rawTick = g.tick;
      let lastF = 0;
      g.tick = function (dt, r) { const a = performance.now(); if (lastF) ft.push(a - lastF); lastF = a; const o = rawTick.call(g, dt, r); tk.push(performance.now() - a); return o; };
      ...
      const frame = { n: ft.length, mean: mean(ft), p95: q(ft, 0.95), max: q(ft, 1), tickMean: mean(tk), tickP95: q(tk, 0.95), tickMax: q(tk, 1) };
      const bodies = g.world.bodies.length, contacts = g.world.contacts.length;
      ...
      console.error = oe;
      return {
        biome: g.biome.current, started: g.state.started, frame, bodies, contacts,
  ...
  await page.evaluate(async (o) => {
    await fetch('/shot?name=l4r-qa-fuzz.json', { method: 'POST',
