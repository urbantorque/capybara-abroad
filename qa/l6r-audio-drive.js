async page => {
  ...
  await page.evaluate(() => {
    ...
    // count every named sfx, and the footstep material
    window.__sfxN = {}; window.__mats = {}; window.__sfxLog = []
    const o = g.sfx
    g.sfx = function (name, opts) {
      ...
      if (window.__sfxLog.length < 400) window.__sfxLog.push([+(performance.now() / 1000).toFixed(1), name, opts && opts.at ? 1 : 0])
  ...
  const meter = (secs) => page.evaluate(async (secs) => {
    ...
    const acc = {}
    for (const k in ans) acc[k] = { f: new Float32Array(N), td: new Float32Array(t.an.fftSize), s2: 0, n: 0, pk: 0, cw: 0, cs: 0, ws2: 0, wn: 0, perSec: [] }
    ...
    while (performance.now() - t0 < secs * 1000) {
      for (const k in ans) {
        ...
        for (let i = 1; i < N; i++) { const h = hz(i); if (h < 20 || h > 20000) continue; const q = Math.pow(10, a.f[i] / 10); a.cw += q * h; a.cs += q }
      }
      if (t.comp.reduction < -1) red++
      try { const m = g.hud.moverAudit(); if (m.live > mv) mv = m.live } catch (e) {}
    ...
    const res = { red, moversMax: mv }
    for (const k in ans) { const a = acc[k]; if (!ans[k]) continue; res[k] = { rms: +(10 * Math.log10(Math.max(1e-12, a.s2 / Math.max(1, a.n)))).toFixed(1), pk: +(20 * Math.log10(Math.max(1e-6, a.pk))).toFixed(1), cent: Math.round(a.cw / Math.max(1e-12, a.cs)), perSec: a.perSec } }
    return res
  }, secs)
  const k = page.keyboard
  const drive = async (secs) => {
    // a naive drive: walk, turn, hop, wheek, grab, run — the first two minutes of somebody who has not read the paper
    const t0 = Date.now()
    let i = 0
  ...
  const chapters = ['sydney', 'kyoto', 'venice']
  for (const ch of chapters) {
    if (ch !== 'sydney') { await page.evaluate((c) => window.__capy.hud.cross(c), ch); await page.waitForTimeout(9000) }
    await page.evaluate(() => { window.__sfxN = {}; window.__mats = {}; window.__sfxLog = []; try { window.__capy.hud.ambAudit(true) } catch (e) {} })
    const row = { biome: await page.evaluate(() => window.__capy.biome.current) }
    row.still = await meter(15)
    const mp = meter(19); await drive(19); row.driveA = await mp
    const mp2 = meter(19); await drive(19); row.driveB = await mp2
    const mp3 = meter(19); await drive(19); row.driveC = await mp3
    const mp4 = meter(19); await drive(19); row.driveD = await mp4
    const mp5 = meter(19); await drive(19); row.driveE = await mp5
    const mp6 = meter(19); await drive(19); row.driveF = await mp6
    row.after = await page.evaluate(() => {
      ...
      const placed = window.__sfxLog.filter(r => r[2]).length
      return { sfx: window.__sfxN, distinct: Object.keys(window.__sfxN).length, mats: window.__mats, placedShare: +(placed / Math.max(1, window.__sfxLog.length)).toFixed(2),
               amb: amb && amb.tally, ambN: amb && amb.total, movers: mov && mov.rows.filter(r => r.live).map(r => r.key + ':' + r.gain.toFixed(2)),
               voiceDrops: mix && mix.voiceDrops, threw: mix && mix.synthThrew, prog: mus && mus.chapProg, secondN: mus && mus.secondN, cells: mus && mus.melCells,
  ...
  await page.evaluate(async (o) => {
    await fetch('/shot?name=l6r-audio-drive.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) })
  }, out)
}
