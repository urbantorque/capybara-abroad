async page => {
        ...
        for (let x = x0; x <= x1; x += STEP) {
          for (let z = z0; z <= z1; z += STEP) {
            ...
            for (const [dx, dz] of DIRS) {
              ...
              const row = [Math.round(x), Math.round(z), Math.round(base*10)/10, Math.round(h*10)/10, id, dx, dz, Math.round(dg*100)/100, Math.round(pd*100)/100, Math.round(it.point.y*10)/10]
              if (rr.hasHit) {
  ...
  await page.evaluate(s => fetch('/shot?name=px-solid-audit-A.json', { method: 'POST', body: s }), bl)
}
