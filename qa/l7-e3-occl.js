async page => {
  ...
  const out = { started: null, chapters: {}, tag: TAG }
  ...
  const sample = async () => page.evaluate(() => {
    ...
    const capB = new T.Vector3(g.capy.group.position.x, g.capy.group.position.y + 0.45, g.capy.group.position.z)
    ...
    for (let j = 0; j < 7; j++) for (let i = 0; i < 5; i++) {
      ...
      if (hit.length) {
        occ++
        // the first hit the capsule does NOT open is what the player sees
        let seen = false
    ...
    return { t: +(performance.now() / 1000).toFixed(1), pitch: +(pitch * 180 / Math.PI).toFixed(1), hz: +hz.toFixed(2), dist: +dist.toFixed(1),
      ...
      occ: n ? +(occ / n).toFixed(2) : null, occSeen: n ? +(occSeen / n).toFixed(2) : null, names, rayMs: +(performance.now() - t0).toFixed(0),
      speed: +(aa.speed || 0).toFixed(1), yaw: +(aa.yaw || 0).toFixed(2), err: g.state.lastError ? String(g.state.lastError).slice(0, 80) : null }
  })
  const CH = ['sydney', 'pasto', 'quay', 'kyoto', 'cali', 'rio', 'iceland', 'sahara', 'drift', 'venice', 'kowloon', 'palawan', 'goreme', 'manly', 'pantanal', 'cave', 'antarctic', 'monaco', 'hanoi']
  for (const c of CH) {
    if (c !== 'sydney') {
