...
const q=vm.createContext({game,post:{},renderer:{getDrawingBufferSize:()=>({x:1280,y:760})},mainPostSize:{},
  ...
  dofA:target('dofA'),dofB:target('dofB'),matBright:{uniforms:{uTexel:uniform}},matCoC:{uniforms:{uTexel:uniform}},matComp:{uniforms:{uPx:uniform}}});
const resize=main.match(/  post.resize = function \(\) \{[^]*?\n  \};/)[0];
vm.runInContext('let vw=0,vh=0,bw=0,bh=0,ww=0,wh=0;'+resize,q);
for(const scale of [1,.8,.6,1]){
  ...
  assert.equal(sizes.filter(s=>s[0]!=='uniform').length,7,'all existing targets resize together');
