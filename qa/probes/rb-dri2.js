async page => {
  ...
  R.gap = await page.evaluate(() => {
    const g = window.__capy; const A = g.drift; const b = g.capy.body;
    // stand on the shelf, then launch flat across 24 m and land back on it
    const s = A.shelf;
  ...
  R.seed = await page.evaluate(() => {
    const g = window.__capy; const A = g.drift; const b = g.capy.body;
    const S = { tries: [] };
    for (let k = 0; k < 8 && !g.taskDone('driftseed'); k++) {
      const sd = A.seed();
      b.position.set(sd.x, sd.y - 1.0, sd.z); b.velocity.set(0, 0, 0);
      b.previousPosition.copy(b.position); b.interpolatedPosition.copy(b.position);
      for (let i = 0; i < 10; i++) g.tick(1 / 60, false);
      g.input.actionPressed = true; g.tick(1 / 60, false);
      S.tries.push([k, A.onSeed()]);
      for (let i = 0; i < 3000; i++) {
        g.tick(1 / 60, false);
        if (!A.onSeed()) break;
        if (g.taskDone('driftseed')) break;
      }
      S.tries[S.tries.length - 1].push(+A.seedRide().toFixed(1));
    }
    S.done = !!g.taskDone('driftseed');
    S.err = g.state.lastError || null;
    return S;
  });
  await page.evaluate(async (o) => {
    await fetch('/shot?name=rb-dri2.json', { method: 'POST',
