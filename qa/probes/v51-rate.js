async page => {
  ...
  const NAMES = ['sydney','venice','manly','monaco']
  const out = []
  ...
  await page.evaluate(o => fetch('/shot?name=v51rate.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) }), out)
}
