async page => {
  ...
  const out = {}
  for (const n of ['quay','goreme','sahara']) {
  ...
  await page.evaluate(async (o) => { await fetch('/shot?name=L1c.json', { method:'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) }) }, out)
}
