async page => {
  ...
  for (const c of cands) {
    ...
    if (out0 > 4) {
      ...
      // NOT scrollIntoView: .capyui-jrcard is a rotated (transform) scroll box and
      // Chromium's scrollIntoView is a no-op inside one, which is what made the
      // first pass call the whole key legend "clipped". Move scrollTop by hand.
      for (const p of sc) {
        const pr = p.getBoundingClientRect()
        let q = el.getBoundingClientRect()
        if (q.bottom > pr.bottom) p.scrollTop += (q.bottom - pr.bottom)
        else if (q.top < pr.top) p.scrollTop += (q.top - pr.top)
        q = el.getBoundingClientRect()
        if (q.right > pr.right) p.scrollLeft += (q.right - pr.right)
        else if (q.left < pr.left) p.scrollLeft += (q.left - pr.left)
      }
      ...
      if (out1 > 4) {
        ...
        if (!seen.has(k)) {
          ...
          res.unreachable.push({ c: cls(el), hidden: Math.round(out1), scrollers: sc.length,
                                 ...
                                 t: c.txt.slice(0, 40), par: el.parentElement ? cls(el.parentElement) : null })
        }
      }
    }
  ...
  const SIZES = [[1280,720],[1366,768],[1600,900]]
  ...
  await page.evaluate(async (o) => { await fetch('/shot?name=b4ui-10.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) }) }, out)
}
