async page => {
  ...
  await page.evaluate(() => {
    ...
    window.__said = []
    const o = g.toast
    g.toast = function (s) { window.__said.push(String(s)); return o.apply(g, arguments) }
    window.__run = async a => {
      ...
      await sleep(250)
      return { tag: a.tag, said: window.__said.slice(), stored: g.hud.ghostAudit ? undefined : undefined }
    }
  })
  const res = []
  const cases = [
  ...
  const board = await page.evaluate(async () => {
    ...
    for (const id of ids) g.hud.completeTask(id)
    await sleep(1200)
    const el = document.querySelector('.capyui-clue')
    return { ids: ids.length, clue: el ? el.textContent : null }
  })
  await page.evaluate(o => fetch('/shot?name=v51a.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o, null, 1)))) }), { res, board })
}
