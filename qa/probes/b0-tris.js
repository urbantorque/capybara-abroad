  ...
  for (const n of ['cave','antarctic']) {
    out[n] = await page.evaluate((name) => {
      ...
      return { tris: Math.round(tris), shadowTris: Math.round(shadowTris), meshes,
               ...
               top: per.slice(0,24), err: g.state.lastError || null }
    }, n)
  }
  await page.evaluate(async (o) => { await fetch('/shot?name=b0tris.json', { method:'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o, null, 1)))) }) }, out)
}
