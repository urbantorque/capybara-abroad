async page => {
  ...
  await cam({ biome:'venice', warm:200, px:-4, py:1.6, pz:-30, cx:-4, cy:2.4, cz:-24, tx:-4, ty:1.2, tz:-34, name:'V2-pigeon.png', fov:38 })
  await cam({ biome:'venice', px:-4, py:1.6, pz:-24, cx:-4, cy:9, cz:-12, tx:-4, ty:5, tz:-56, name:'V2-piazza.png' })
  await cam({ biome:'venice', warm:150, px:-4, py:1.6, pz:13, cx:-4, cy:7, cz:24, tx:-4, ty:3, tz:-6, name:'V2-arrival.png' })
  await cam({ biome:'venice', px:-17.8, py:1.0, pz:-20, cx:-12, cy:3.6, cz:-13, tx:-19, ty:1.6, tz:-24, name:'V2-cafe.png' })
  const err = await page.evaluate(() => window.__capy.state.lastError ? String(window.__capy.state.lastError) : 'none')
  await page.evaluate(async (o) => { await fetch('/shot?name=v2err.json', { method:'POST', body: btoa(String(o)) }) }, err)
}
