async page => {
    ...
    out[n] = await page.evaluate((name) => {
      ...
      for (let x = B[0]; x <= B[1]; x += 2.5) for (let z = B[2]; z <= B[3]; z += 2.5) {
        ...
        for (const d of dirs) {
          ...
          pts.push([Math.round(h.point.x), Math.round(h.point.y*10)/10, Math.round(h.point.z), (h.object.isInstancedMesh?'I'+h.object.count:'M')+':'+(h.object.geometry.boundingBox||(h.object.geometry.computeBoundingBox(),h.object.geometry.boundingBox)).min.toArray().map(Math.round).join(',')+'|'+h.object.geometry.boundingBox.max.toArray().map(Math.round).join(',')])
        }
      }
  ...
  await page.evaluate(async (o) => {
    await fetch('/shot?name=t4solid.json', { method:'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) })
  }, out)
}
