async page => {
    ...
    const physD = (x, y, z, ux, uz, R) => {
      ...
      try {
        g.world.raycastAll(F, Tv, {}, (res) => {
          ...
          // ...AND NOT THE GROUND. A horizontal ray at chest height on any slope
          // hits the terrain heightfield, and you do not STOP at a slope, you
          // walk up it — so a control picked that way is not a wall, which is
          // why every chapter with relief reported its own control as a
          // pass-through while flat Kowloon's stopped dead.
  ...
  await page.evaluate(o => fetch('/shot?name=px-x5-walk4.json', {
