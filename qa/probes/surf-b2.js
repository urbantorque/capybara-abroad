async page => {
  ...
  const sink = async () => { try { await page.evaluate(o => fetch('/shot?name=surf-b.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) }), out); } catch (e) { out.errs.push('SINK ' + String(e).slice(0, 100)); } };

  out.flowMax = await ev(`const r={}; for(const n of ['cave','kyoto','manly','pantanal','rio']){ sw(n); const a=api(); r[n]=flowMax(a,260,5); } return r;`);
  await sink();
  out.manly1 = await ev(`
    ...
    r.againstRip = swimTest(-34,-5,[0,1],480);
    r.acrossRip = swimTest(-34,-5,[1,0],480);
    return r;`);
  await sink();
  out.manly2 = await ev(`sw('manly'); const r={}; r.driftNoInput = swimTest(-34,-5,[0,0],1500); return r;`);
  await sink();
  out.manly3 = await ev(`sw('manly'); const x0=b.position.x, z0=b.position.z; let jumps=0, lx=x0, lz=z0; for(let i=0;i<1500;i++){ T(1); const d=Math.hypot(b.position.x-lx,b.position.z-lz); if(d>3) jumps++; lx=b.position.x; lz=b.position.z; } return { end:[r2(b.position.x), r2(b.position.z), r2(b.position.y)], swim:g.capy.swimming, jumps, wyEnd:r2(wy(b.position.x,b.position.z)), over:api().isOverWater(b.position.x,b.position.z) };`);
  await sink();
  out.kyoto = await ev(`
    sw('kyoto'); const a=api(); const m=flowMax(a,260,4); const r={m};
    if(m.at){ const fx=m.at[2], fz=m.at[3], s=Math.hypot(fx,fz)||1; const ux=fx/s, uz=fz/s;
      r.against = swimTest(m.at[0],m.at[1],[-ux,-uz],480); r.with = swimTest(m.at[0],m.at[1],[ux,uz],240); }
    return r;`);
  await sink();
  out.cave = await ev(`
    sw('cave'); const a=api(); const m=flowMax(a,200,4); const r={m};
    if(m.at){ const fx=m.at[2], fz=m.at[3], s=Math.hypot(fx,fz)||1; const ux=fx/s, uz=fz/s;
      r.against = swimTest(m.at[0],m.at[1],[-ux,-uz],480); r.none = swimTest(m.at[0],m.at[1],[0,0],600); }
    return r;`);
  await sink();
  out.slope = await ev(`
    sw('pasto'); const a=api(); const r={};
    let pick=null; for(let x=-200;x<=200;x+=5) for(let z=-200;z<=200;z+=5){ if(a.isOverWater(x,z)) continue; if(a.navBlocked&&a.navBlocked(x,z,0.34)) continue; const gx=(th(x+2,z)-th(x-2,z))/4; const gz=(th(x,z+2)-th(x,z-2))/4; const gr=Math.hypot(gx,gz); if(gr>0.28 && gr<0.34 && (!pick||Math.abs(gx)>Math.abs(pick.gx))) pick={x,z,gx:r2(gx),gz:r2(gz),gr:r2(gr)}; }
  ...
  out.rain = await ev(`
    ...
    for(let i=0;i<3600;i++){ T(1); if(i%600===0) r.rows.push([r2(W.wetness()), r2(W.slip()), r2(g.capy.slip||0), r2(Math.hypot(W.gust().x,W.gust().z))]); }
    return r;`);
  await sink();
  out.venice = await ev(`
    sw('venice'); const a=api(); const r={ rows:[] }; let mn=1e9,mx=-1e9; let f=null;
    for(let x=-120;x<=120&&!f;x+=4) for(let z=-120;z<=120&&!f;z+=4){ if(a.isOverWater(x,z) && a.isOverWater(x+3,z) && a.isOverWater(x-3,z) && a.isOverWater(x,z+3) && a.isOverWater(x,z-3)) f=[x,z]; }
    r.at=f; if(f) placeW(f[0],f[1]);
    for(let i=0;i<3600;i++){ T(1); const w=a.waterLevel; mn=Math.min(mn,w); mx=Math.max(mx,w); if(i%600===0) r.rows.push([r2(w), r2(b.position.y), r2(b.position.y-w), g.capy.swimming]); }
    r.tide=[r2(mn),r2(mx)]; return r;`);
  await sink();
  out.monaco = await ev(`
    sw('monaco'); const a=api(); const r={ rows:[] }; T(600);
    const c=a.car(); place(c.x, c.z, 1.6); let lx=c.x, lz=c.z;
    for(let i=0;i<300;i++){ T(1); const cc=a.car(); const v=Math.hypot(cc.x-lx,cc.z-lz)/D; lx=cc.x; lz=cc.z; if(i%20===0) r.rows.push([r2(v), r2(g.capy.frameVX||0), r2(g.capy.frameVZ||0), r2(Math.hypot(b.position.x-cc.x,b.position.z-cc.z)), r2(b.position.y-cc.y), r2(spd())]); }
    return r;`);
  await sink();
