async page => {
  ...
  for (const n of names) {
    ...
    out[n] = await page.evaluate((name) => {
      ...
      const B = { manly:[-100,120,-70,80], pantanal:[-120,120,-115,100] }[name]
      const api = g[name]
  ...
  await page.evaluate(async (o) => {
    await fetch('/shot?name=f7solid.json', { method:'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) })
  }, out)
}
