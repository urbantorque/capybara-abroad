async page => {
  ...
  const out = await page.evaluate(async () => {
    ...
    let wg = null; g.scene.traverse(o => { if (o.name === 'quayWakeGulls') wg = o; });
    res.wg = !!wg;
    // where are they relative to the boat?
    if (wg) {
      const m = new g.THREE.Matrix4(), v = new g.THREE.Vector3(), d = [];
      for (let i = 0; i < wg.count; i++) { wg.getMatrixAt(i, m); v.setFromMatrixPosition(m);
        d.push(+Math.hypot(v.x - bp.x, v.z - bp.z).toFixed(1)); }
      res.dist = d;
    }
    await shoot('Z-wake2', bp.x + 22, 14, bp.z + 34, bp.x, 4, bp.z - 4);
    up('KeyW');
    res.err = g.state.lastError || null;
    return res;
  });
  await page.evaluate(async (o) => {
    await fetch('/shot?name=z9.json', { method: 'POST',
