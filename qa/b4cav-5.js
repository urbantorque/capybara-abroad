async page => {
  ...
  await page.evaluate(() => {
    ...
    g.frameShot({ yaw: 0, dist: 12, pitch: 13 * Math.PI / 180, raise: 0.6, hold: 90 });
  });
  ...
  const out = await page.evaluate(() => {
    ...
    return { capy: [+cp.x.toFixed(2), +cp.y.toFixed(2), +cp.z.toFixed(2)],
             ...
             framing: +g.framing().toFixed(3), sky: +g.cave.skyward().toFixed(2) };
  });
  await page.evaluate(async o => {
    await fetch('/shot?name=b4cav-5.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) });
  }, out);
}
