async page => {
  ...
  const out = await page.evaluate(() => {
    ...
    const cand = [
      { yaw: -Math.PI / 2, pitch: 0.05, raise: 0, dist: null, tag: 'shipped' },
      { yaw: -1.48, pitch: -0.02, raise: 1.6, dist: 17, tag: 'level, look up a little' },
      { yaw: -1.48, pitch: -0.10, raise: 3.0, dist: 18, tag: 'below, envelope + sky' },
      { yaw: -1.48, pitch: 0.02, raise: 0.8, dist: 20, tag: 'wide, level' },
      { yaw: -1.30, pitch: -0.05, raise: 2.2, dist: 18, tag: 'quarter, below' },
    ]
  ...
  await page.evaluate(async (o) => { await fetch('/shot?name=b4-gorsun.json', { method: 'POST', body: btoa(unescape(encodeURIComponent(JSON.stringify(o)))) }) }, out)
}
