async page => {
  ...
  for (const c of cases) {
    ...
    const title = await page.evaluate(() => ({ has: !!window.__capy, running: !!window.__capyRunning, err: (document.getElementById('err') || {}).textContent || '',
      ...
      bad: (function () { try { return localStorage.getItem('capy3.journey.broken.v1') } catch (e) { return 'ERR' } })() }))
    ...
    await page.screenshot({ path: 'qa/l6-save-' + c.tag + '.png' })
  ...
  await page.evaluate(async (o) => {
    await fetch('/shot?name=l6-save.json', { method: 'POST',
