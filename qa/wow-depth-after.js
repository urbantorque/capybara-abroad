async page => {
  ...
  for (let i = 0; i < 19; i++) {
    ...
    try {
      ...
      const row = await page.evaluate(() => {
        ...
        // grass (the swayD-keyed instanced fans), the mist sheets (renderOrder
        // high, transparent, depthWrite off) and the weather fields are VOLUMES
        // and veils, not planes: a ray they stop was headed for the ground
        // behind them. Counted separately so the planes read as planes.
        ...
        const run = (skipVeil) => {
          ...
          for (let yy = 0; yy < 9; yy++) for (let xx = 0; xx < 13; xx++) {
            ...
            const hits = rc.intersectObjects(g.scene.children, true).filter(h => vis(h.object) && h.object.renderOrder !== -20 && !(skipVeil && veil(h.object)))
            n++
        ...
        return { biome: g.biome.current, bins: run(false), planes: run(true) }
      })
      row.want = NAMES[i]
      out.push(row)
    } catch (e) {
  ...
  await page.evaluate(o => fetch('/shot?name=wow-depth-after.json', { method: 'POST',
