...
function cavMerger() {
  ...
  const M = {
    ...
    box(cx, cy, cz, sx, sy, sz, color, rx, ry, rz) {
      return M.add(cavG.box, cavXform(cx, cy, cz, rx || 0, ry || 0, rz || 0, sx, sy, sz), color);
    },
    cyl(cx, cy, cz, r, h, color, rx, ry, rz, seg) {
      const g = seg === 4 ? cavG.cyl4 : seg === 8 ? cavG.cyl8 : cavG.cyl6;
      return M.add(g, cavXform(cx, cy, cz, rx || 0, ry || 0, rz || 0, r * 2, h, r * 2), color);
    },
    cone(cx, cy, cz, r, h, color, rx, ry, rz, seg) {
      const g = seg === 8 ? cavG.cone8 : cavG.cone6;
      return M.add(g, cavXform(cx, cy, cz, rx || 0, ry || 0, rz || 0, r * 2, h, r * 2), color);
    },
    sph(cx, cy, cz, rx2, ry2, rz2, color, seg) {
      return M.add(seg === 8 ? cavG.sph8 : cavG.sph6,
                   cavXform(cx, cy, cz, 0, 0, 0, rx2 * 2, ry2 * 2, rz2 * 2), color);
    },
    build() {
