...
function quayMerger() {
  ...
  const M = {
    ...
    box(cx, cy, cz, sx, sy, sz, color, rx, ry, rz) {
      return M.add(quayG.box, quayXform(cx, cy, cz, rx || 0, ry || 0, rz || 0, sx, sy, sz), color);
    },
    cyl(cx, cy, cz, r, h, color, rx, ry, rz, seg) {
      const g = seg === 4 ? quayG.cyl4 : seg === 8 ? quayG.cyl8 : quayG.cyl6;
      return M.add(g, quayXform(cx, cy, cz, rx || 0, ry || 0, rz || 0, r * 2, h, r * 2), color);
    },
    cone(cx, cy, cz, r, h, color, rx, ry, rz, seg) {
      const g = seg === 4 ? quayG.cone4 : quayG.cone6;
      return M.add(g, quayXform(cx, cy, cz, rx || 0, ry || 0, rz || 0, r * 2, h, r * 2), color);
    },
    sph(cx, cy, cz, rx2, ry2, rz2, color) {
      return M.add(quayG.sph6, quayXform(cx, cy, cz, 0, 0, 0, rx2 * 2, ry2 * 2, rz2 * 2), color);
    },
    vert(x, y, z, color) {
...
function quayVC() {
  return grain(mat(0xffffff, { vertexColors: true }),
               { scale: 0.45, amount: 0.09, warp: 0.5, near: 0.34, nearScale: 6 });
}
